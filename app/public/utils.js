...
//把数据库传来的如下结构的数据，转换成Ant Design的Tree，Menu所需的树状格式。
/*
var data=[  
  {_id:1,pid:0,username:'A'},  //pid, 父节点的_id
  {_id:2,pid:4,username:"E[父C]"},  
  {_id:3,pid:7,username:"G[父F]"},  
  {_id:4,pid:1,username:"C[父A]"},  
  {_id:5,pid:6,username:"D[父B]"},  
  {_id:6,pid:0,username:'B'},  
  {_id:7,pid:4,username:"F[父C]"}  
  ];  
*/
//关键变量pos，key-value对对象，用于保存每个已添加到tree中的节点在tree中位置信息，
//比如上面测试数据父节点A添加到tree后，那么pos中增加一条数据，pos={"1":[0]}，
//key=1就是父节点A的_id，这样写便于查找，value=[0]表示父节点A是tree的第一个元素，即tree[0]，
...
function toTreeData(data){  
  ...
  while(data.length!=0){  
      if(data[i].pid == undefined || null){ //这是顶级节点 
          ...
          var posArr=pos[data[i].pid];  //posArr = GET 当前节点的父节点的在tree中位置数组，形如[0,3]
