  ...
  async postImplement() {
    ...
    if (ctx.isAuthenticated()) {
      try{
        ...
        const event = {user: i.user, pid: i.pid, sid: i._id, action: i.state,
                        name: '[实现计划项]' + (i.tags[0] ? '标签:' + i.tags[0].username+',' : '' )
                        + '年度:' +i.budgetyear + ',标的:' + i.name
                        +',数量:' + i.quantity + ',金额:' + i.amount.toFix(2) + '（万元）' //，实现状态：[ ' + i.state + ' ]。'
                      };
